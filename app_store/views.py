...
def vip_info(request):
    ...
    for item in user_list:
        temp = {'id': item.id, 'name': item.name, 'mail': item.mail, 'password': item.password,
                'birthday': item.birthday, 'level': item.level, 'exp': item.exp, 'money': item.money,
                'expireDate': item.expire_date.timestamp(), 'createDate': item.create_date.timestamp()}
...
def vip_recharge(request):
    ...
    try:
        ...
        user.save()
        vip_order = VipOrder.objects.create(vip_id=user, date=timezone.now(), total_price=post['money'],
                                            pay_price=post['money'], status=1)
        vip_order.save()
        deal_type = DealType.objects.filter(title='vip').first()
        deal = Deal.objects.create(type=deal_type,
